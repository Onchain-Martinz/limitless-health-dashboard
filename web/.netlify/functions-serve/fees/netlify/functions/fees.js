...
// netlify/functions/fees.ts
var PROTOCOL = "limitless-exchange";
var FEES_URL = `https://api.llama.fi/summary/fees/${PROTOCOL}?dataType=dailyFees`;
var REQUEST_TIMEOUT_MS = 1e4;
var handler = async () => {
  try {
    const response = await axios_default.get(FEES_URL, { timeout: REQUEST_TIMEOUT_MS });
    ...
    for (const entry of raw) {
      ...
      points.push({
        date: new Date(unixSeconds * 1e3).toISOString().slice(0, 10),
        dailyFees,
